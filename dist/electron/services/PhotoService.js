"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PhotoService = void 0;
const electron_log_1 = __importDefault(require("electron-log"));
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const uuid_1 = require("uuid");
const sharp_1 = __importDefault(require("sharp"));
const electron_1 = require("electron");
const connection_1 = require("../database/connection");
class PhotoService {
    constructor() {
        this.db = (0, connection_1.getDatabase)();
    }
    //get default save directory
    getDefaultSaveDurectrory() {
        const userDataPath = electron_1.app.getPath('userData');
        return path_1.default.join(userDataPath, 'photos');
    }
    //save a new photo
    async savePhoto(input) {
        const photoId = (0, uuid_1.v4)();
        const timestamp = new Date().toISOString();
        const dateFolder = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
        try {
            //see if the directory to save photos exists
            const settingsRow = this.db.prepare("SELECT save_directory, photo_format, photo_quality FROM settings WHERE id = 1")
                .get();
            let saveDirectory = settingsRow?.save_directory || this.getDefaultSaveDurectrory();
            const photoDir = path_1.default.join(saveDirectory, dateFolder);
            //ensure directory exists
            if (!fs_1.default.existsSync(photoDir)) {
                fs_1.default.mkdirSync(photoDir, { recursive: true });
            }
            //Gebnerate randomm filename
            const randomString = Math.random().toString(36).substring(2, 8);
            const format = settingsRow?.photo_format || 'jpg';
            const filename = `photo_${Date.now()}_${randomString}.${format}`;
            const filepath = path_1.default.join(photoDir, filename);
            const thumbnailPath = path_1.default.join(photoDir, `thumb_${filename}`);
            //Process and save the image using sharp
            let image = (0, sharp_1.default)(input.imageBuffer);
            const metaData = await image.metadata();
            await image
                .toFormat(format, { quality: settingsRow?.photo_quality || 80 })
                .toFile(filepath);
            // Generate thumbnail
            await (0, sharp_1.default)(input.imageBuffer)
                .resize(300, 300, { fit: 'cover' })
                .jpeg({ quality: 80 })
                .toFile(thumbnailPath);
            // Get file size
            const stats = fs_1.default.statSync(filepath);
            // Save to database
            const photo = {
                id: photoId,
                session_id: input.session_id,
                filename,
                filepath,
                thumbnail_path: thumbnailPath,
                width: metaData.width || 0,
                height: metaData.height || 0,
                file_size: stats.size,
                taken_at: timestamp,
                layout_type: input.layout_type || 'single',
                metadata: input.metadata ? JSON.stringify(input.metadata) : undefined,
            };
            this.db.prepare(`
        INSERT INTO photos (
          id, session_id, filename, filepath, thumbnail_path,
          width, height, file_size, taken_at, layout_type, metadata
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(photo.id, photo.session_id || null, photo.filename, photo.filepath, photo.thumbnail_path, photo.width, photo.height, photo.file_size, photo.taken_at, photo.layout_type, photo.metadata || null);
            electron_log_1.default.info(`Photo saved: ${photoId} at ${filepath}`);
            return photo;
        }
        catch (error) {
            electron_log_1.default.error('Error saving photo:', error);
            throw new Error(`Failed to save photo: ${error.message}`);
        }
    }
    async getAllPhotos(limit = 100, offset = 0) {
        try {
            const photos = this.db
                .prepare(`
                    SELECT * FROM photos 
                    ORDER BY taken_at DESC 
                    LIMIT ? OFFSET ?
                `)
                .all(limit, offset);
            return photos;
        }
        catch (error) {
            electron_log_1.default.error('Error getting all photos:', error);
            throw error;
        }
    }
    /**
     * Get photo by ID
     */
    async getPhotoById(id) {
        try {
            const photo = this.db
                .prepare('SELECT * FROM photos WHERE id = ?')
                .get(id);
            return photo || null;
        }
        catch (error) {
            electron_log_1.default.error(`Error getting photo ${id}:`, error);
            throw error;
        }
    }
    /**
     * Delete a photo
     */
    async deletePhoto(id) {
        try {
            const photo = await this.getPhotoById(id);
            if (!photo) {
                throw new Error('Photo not found');
            }
            // Delete files
            if (fs_1.default.existsSync(photo.filepath)) {
                fs_1.default.unlinkSync(photo.filepath);
            }
            if (photo.thumbnail_path && fs_1.default.existsSync(photo.thumbnail_path)) {
                fs_1.default.unlinkSync(photo.thumbnail_path);
            }
            // Delete from database
            this.db.prepare('DELETE FROM photos WHERE id = ?').run(id);
            electron_log_1.default.info(`Photo deleted: ${id}`);
            return true;
        }
        catch (error) {
            electron_log_1.default.error(`Error deleting photo ${id}:`, error);
            throw error;
        }
    }
}
exports.PhotoService = PhotoService;
//# sourceMappingURL=PhotoService.js.map